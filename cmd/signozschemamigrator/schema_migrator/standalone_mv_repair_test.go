...
func TestPlanStandaloneMVs(t *testing.T) {
	...
	t.Run("an-mv-writing-to-an-alias-view-is-rebuilt-onto-storage", func(t *testing.T) {
		// usage_explorer_mv points at distributed_usage_explorer, which in this
		// layout is the VIEW, not the table.
	...
	t.Run("recreates-missing-metrics-rollups-onto-their-storage-tables", func(t *testing.T) {
		// Storage and alias tables in place, all five rollup MVs missing. Each
		// is recreated onto whichever table of its pair is storage, and the
		// chained rollups read storage too.
